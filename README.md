...
* If you are able to use Linux (or if you use windows then you can use WSL or Ubuntu) then download the Linux version, it has the most up to date UI dependencies and it is able to render the zoom in and zoom out, move up, down, left, and right,  functionalities for more ease of use and user interaction. In addition, it will also enable you to be able to save the actual images of the measurments and sperm overlay windows in order to save your work and see the results if you ever want to look at them again. You install the file and run ./FINAL_BIO_PROJECT_LINUX 

* If you have to use the windows exe file, then it will still work for being able to do the measuring functionalities, Due to limitations in executable file generation for windows exes and the fact I used the cv2 dependency for my UI, it will work but it will not be as robust as the linux version. There is also a dependency that is present within the zip that also has to be there when it runs: You install the file and run ./FINAL_BIO_PROJECT_WINDOWS.exe

* If you prefer to clone the repository, then you can look at the RULER - source code and requirments folder to look at requirements.txt to see all of the dependencies required for this application and attempt to build and run them all yourself as well. The only one that may be annoying to figure out is cv2 but everything else should be runnable unless you run into additional issues. You can try
